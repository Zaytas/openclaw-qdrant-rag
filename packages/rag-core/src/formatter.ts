...
import type { SearchResult, RecallResponse, ConfidenceLabel } from './types.js';
...
function confidenceEmoji(confidence: ConfidenceLabel): string {
  switch (confidence) {
    case 'high':
      return '🟢';
    case 'medium':
...
export function formatForInjection(
  ...
  options: { maxTokens: number; hardCapTokens: number },
): string {
 ...
 */
function trimBlock(result: SearchResult, maxTokens: number): string {
 ...
 */
export function formatForCli(response: RecallResponse): string {
  const lines: string[] = [];
