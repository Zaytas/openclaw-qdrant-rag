...
import type { SearchResult, RagConfig, ConfidenceLabel, SourceType } from './types.js';
...
function chunkKey(result: SearchResult): string {
  if (!result.file) return `${result.sourceType}:${result.source}`;
...
function isRecent(timestamp: string | undefined, windowDays: number): boolean {
...
function getSourceWeight(
  ...
  weights: RagConfig['sourceWeights'],
): number {
...
export function rankAndDedup(
  ...
  const grepByKey = new Map<string, SearchResult>();
